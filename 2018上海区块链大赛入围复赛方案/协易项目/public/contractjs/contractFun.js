function addProtocol(userName, title, protocolText, signersSize) {
  return new Promise((resolve, reject) => {
    myContractInstance.addProtocol(userName, title, protocolText, signersSize, function(err, res) {
      if (err) {
        reject(err)
      } else {
        resolve(res)
...
//显示所有协议的所有信息
async function showAllProtocolAllInfo(userName) {
  var id = await getUserProtocolSize(userName);
  var info
  var size
  var allSigner
  var allInfo = []
  console.log(id);
  for (let j = 0; j < id; j++) {
    info = await showProtocol(userName,j);
    size = await showProtocolSignersSize(userName, j);
    size = size.c[0]
    allSigner = [];
    for (let i = 0; i < size; i++) {
      var signer = await showProtocolSigners(userName, j, i);
      allSigner.push(signer)
    }
    allInfo.push({
      info: info,
